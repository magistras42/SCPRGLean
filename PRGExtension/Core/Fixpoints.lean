...
-- OUTDATED - We needed to decouple the set evolution function from view generation due to increased complexity of functions using fixpoint calcs
...
lemma fixaccess {α : Type} {Z : Type} [DecidableEq α]
  -- View generator (e.g., fun keys => hideEncrypted keys expr)
  (f1 : Finset α → Z)
  -- Unified step function (e.g., keyRecovery expr)
  (f : Finset α → Finset α)
  -- Monotonicity of the unified step
  (f_monotone : ∀ X Y, X ⊆ Y → f X ⊆ f Y)
  -- Bound elements
  (fBound : Z)
  (boundSet : Finset α)
  (HfBound : ∀ x, f x ⊆ boundSet)
  -- Relation properties - stronger because doesn't need reflexivity
  (R : Z → Z → Prop)
  (RTrans : ∀ {x y z}, R x y → R y z → R x z)
  -- 6. Assumption that a single step maintains the relation
  (Ras : ∀ x, f x ⊆ x → R (f1 x) (f1 (f x)))
  (Rsup : R fBound (f1 boundSet)) :
  -- Conclusion: The relation holds between the original expression and the view at the fixpoint
  R fBound (f1 (greatestFixpoint f f_monotone boundSet HfBound)) :=
by
  simp [greatestFixpoint]
  let rec gfixL2 (S : Finset α) (X : S ⊆ boundSet) (Y : f S ⊆ S) (Hnew : R fBound (f1 S)) :
  R fBound (f1 (greatestFixpoint.gfix f f_monotone boundSet HfBound S X Y)) :=
    if h2 : f S = S then by simp[greatestFixpoint.gfix, h2]; assumption else (by
      have Htmp := gfixL2 (f S) (HfBound S) (f_monotone _ _ Y) (RTrans Hnew (Ras S Y))
      rw [greatestFixpoint.gfix]
      simp [h2]
      assumption
    )
  ...
  apply gfixL2 boundSet (refl boundSet) (HfBound boundSet) Rsup
